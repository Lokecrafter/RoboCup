...
class LineFinder {
    ...
    public:
        ...
        //threshold - threshold that must be crossed before linefinder is sure that it's following a line
        void begin(int numPins, int* photoResistorPins, Vector2* photoPositions, int* whiteValues, int* blackValues, float threshold = 0.5){
        ...
        //blackValues - array of calibrated values for when linefinder sees black
        //threshold - threshold that must be crossed before linefinder is sure that it's following a line
        void begin(int numPins, int* photoResistorPins, VectorP* photoAngles, int* whiteValues, int* blackValues, float threshold = 0.5){
            //Convert photoAngles from degrees to radians and then into 2D-vector
            Vector2 photoPositions[numPins];
            for(int i = 0; i < numPins; i++){
                photoAngles[i].toDegrees();
        ...
        //Returns the normalized polar 2D-vector towards the line. Vector is in radians
        VectorP getLineAng(){                                                           
            int index = 0;
            float maxVal = 0;
            //Find max value
        ...
        //Returns the normalized cartesian 2D-vector towards the line
        Vector2 getLineVect(){                                                           
            int index = 0;
            float maxVal = 0;
            //Find max value
            ...
            avrgAngle /= sumValues; //Definition for center of mass equation, exept this time it's a center of value equation
            return((Vector2)VectorP(avrgAngle, 1));
        }
};
#endif